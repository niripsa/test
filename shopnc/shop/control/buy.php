...
class buyControl extends BaseBuyControl {
    ...
    public function buy_step1Op() {
        ...
        //$_POST：array(3) { ["act"]=> string(3) "buy" ["op"]=> string(9) "buy_step1" ["cart_id"]=> array(1) { [0]=> string(4) "52|1" } }

        //得到购买数据
        $logic_buy = Logic('buy');
        $result = $logic_buy->buyStep1($_POST['cart_id'], $_POST['ifcart'], $_SESSION['member_id'], $_SESSION['store_id']);
        //这里$result储存着商品信息和订单信息
    ...
    public function payOp() {
        //下面是post数据
        //cart_id[]=52|1&pay_message[1]=&act=buy&op=buy_step2&ifcart=&pay_name=online&vat_hash=58-gzuwdm8CN507TpE8viCjA7zl5--Gu_as&address_id=1&buy_city_id=224&allow_offpay=0&allow_offpay_batch=1:0&offpay_hash=ex32LczrUwzmC8wGP9zH9Bbrk4gBI5_lK-r5o-h&offpay_hash_batch=nwYbCnuMD4o6sMmtXiuhaAcQU8kfUCqslzj_YUb8mILXYbX&invoice_id=&ref_url=http://127.0.0.1/shop/index.php?act=goods&op=index&goods_id=52
        $pay_sn	= $_GET['pay_sn'];
        //http://127.0.0.1/shop/index.php?act=buy&op=pay&pay_sn=650548450834655002
        ...
        //↑这是一个对象
        //下面搜索必须要同时有单号和session,以确保登陆后支付
        $pay_info = $model_order->getOrderPayInfo(array('pay_sn'=>$pay_sn,'buyer_id'=>$_SESSION['member_id']),true);
        //↑array(4) { ["pay_id"]=> string(1) "4" ["pay_sn"]=> string(18) "650548450834655002" ["buyer_id"]=> string(1) "2" ["api_pay_state"]=> string(1) "0" }
        if(empty($pay_info)){
            showMessage(Language::get('cart_order_pay_not_exists'),'index.php?act=member_order','html','error');
        }
        ...
        $condition['order_state'] = array('in',array(ORDER_STATE_NEW,ORDER_STATE_PAY)); //(in,array(10,20))
        ...
        //↑array(1) { [0]=> array(7) { ["order_id"]=> string(1) "4" ["order_state"]=> string(2) "10" ["payment_code"]=> string(6) "online" ["order_amount"]=> string(5) "99.00" ["rcb_amount"]=> string(4) "0.00" ["pd_amount"]=> string(4) "0.00" ["order_sn"]=> string(16) "9000000000000401" } }
        if (empty($order_list)) {
        ...
        //$order_list=array(1) { [0]=> array(8) { ["order_id"]=> string(1) "4" ["order_state"]=> string(2) "10" ["payment_code"]=> string(6) "online" ["order_amount"]=> string(5) "99.00" ["rcb_amount"]=> string(4) "0.00" ["pd_amount"]=> string(4) "0.00" ["order_sn"]=> string(16) "9000000000000401" ["payment_state"]=> string(12) "在线支付" } }
