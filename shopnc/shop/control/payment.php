...
class paymentControl extends BaseHomeControl{
	...
	public function real_orderOp(){
        ...
        //$payment_info = array(5) { ["payment_id"]=> string(1) "6" ["payment_code"]=> string(5) "wxpay" ["payment_name"]=> string(12) "微信支付" ["payment_config"]=> array(3) { ["appid"]=> string(0) "" ["mchid"]=> string(0) "" ["key"]=> string(0) "" } ["payment_state"]=> string(1) "1" } 支付方式信息
	...
	private function _api_pay($order_info, $payment_info) {
    	...
    	if($payment_info['payment_code'] == 'chinabank') {
        	...
        	//wxpay要求信息用post的方式传过去而不是get
            if (!extension_loaded('curl')) {
                showMessage('系统curl扩展未加载，请检查系统配置', '', 'html', 'error');
            }
            //用html构造一个表单，来发出post请求
            Tpl::setDir('buy');
    	...
    	exit();
    	//string(654) "https://mapi.alipay.com/gateway.do?_input_charset=UTF-8&body=650548450834655002&extend_param=isv%5Esh32&extra_common_param=real_order&logistics_payment=BUYER_PAY&logistics_type=EXPRESS¬ify_url=http%3A%2F%2F127.0.0.1%2Fshop%2Fapi%2Fpayment%2Falipay%2Fnotify_url.php&out_trade_no=650548450834655002&payment_type=1&price=99&quantity=1&receive_address=N&receive_mobile=N&receive_name=testtest&receive_phone=N&receive_zip=N&return_url=http%3A%2F%2F127.0.0.1%2Fshop%2Fapi%2Fpayment%2Falipay%2Freturn_url.php&service=create_direct_pay_by_user&subject=%E5%AE%9E%E7%89%A9%E8%AE%A2%E5%8D%95_650548450834655002&sign=fdbf5201ea4cbef326822a3c145c262a&sign_type=MD5"
	}
    ...
    public function query_stateOp() {
    	//这里的$_GET['pay_id']是字符串,不会执行其中的语句		
